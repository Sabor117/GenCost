...
trait_1 = "GP"
trait_2 = "prescription"

### PICK TWO TRAITS FOR MAIMI PLOTS
### PHENO 1 AND 2

all_files_1 = Sys.glob("all_regenie_files.regenie")

### PICK SECOND PHENO

all_files_2 = Sys.glob("all_regenie_files.regenie")

### Create full set of sum-stats for PHENO 1 - removing SNPs below "p_val_thresh"

curr_pheno_1 = fread(all_files_1[1], data.table = FALSE)

p_val_thresh = -log10(0.05)

curr_pheno_1 = curr_pheno_1[curr_pheno_1$LOG10P > p_val_thresh,]

for (i in 2:length(all_files_1)){

    curr_chr_pheno = fread(all_files_1[i], data.table = FALSE)

    chr = unique(curr_chr_pheno$CHROM)

    cat(paste0("Number of lines in chromosome file ", chr, " = ", nrow(curr_chr_pheno), "\n\n"))

    curr_chr_pheno = curr_chr_pheno[curr_chr_pheno$LOG10P > p_val_thresh,]

    cat(paste0("Number of lines after processing = ", nrow(curr_chr_pheno), "\n\n"))

    curr_pheno_1 = rbind(curr_pheno_1, curr_chr_pheno)

}

### Create full set of sum-stats for PHENO 2 - removing SNPs below "p_val_thresh"

curr_pheno_2 = fread(all_files_2[1], data.table = FALSE)

curr_pheno_2 = curr_pheno_2[curr_pheno_2$LOG10P > p_val_thresh,]

for (i in 2:length(all_files_2)){

    curr_chr_pheno = fread(all_files_2[i], data.table = FALSE)
    ...
    curr_pheno_2 = rbind(curr_pheno_2, curr_chr_pheno)

}

### ALLELE FILTERING

print(nrow(curr_pheno_1))
print(nrow(curr_pheno_2))

curr_pheno_1 = curr_pheno_1[curr_pheno_1$LOG10P > -log10(0.01),]
curr_pheno_1 = curr_pheno_1[curr_pheno_1$A1FREQ >= 0.01,]
curr_pheno_1 = curr_pheno_1[curr_pheno_1$INFO >= 0.6,]
curr_pheno_1$PVAL = 10^(-curr_pheno_1$LOG10P)

curr_pheno_2 = curr_pheno_2[curr_pheno_2$LOG10P > -log10(0.01),]
curr_pheno_2 = curr_pheno_2[curr_pheno_2$A1FREQ >= 0.01,]
curr_pheno_2 = curr_pheno_2[curr_pheno_2$INFO >= 0.6,]
curr_pheno_2$PVAL = 10^(-curr_pheno_2$LOG10P)
...
green_set = c("#006d2c", "#238b45", "#41ab5d") # GP
...
pdf(paste0("/scratch/project_2007428/projects/prj_001_cost_gwas/outputs/figures/manhattan_plots/UKB/", trait_1, "_and_", trait_2, "_costs_", cohort, ".pdf"), width = 30, height = 20)
#par(mar=c(5.1,5.1,4.1,2.1))

miami(x = curr_pheno_1,
    y = curr_pheno_2,
    chr = "CHROM", bp = "GENPOS", snp = "ID", p = "PVAL", 
    genomewideline = -log10(5e-8),
    ymin = -25,
...
png(paste0("/scratch/project_2007428/projects/prj_001_cost_gwas/outputs/figures/manhattan_plots/UKB/", trait_1, "_and_", trait_2, "_costs_", cohort, ".png"), width = 2500, height = 1000)
#par(mar=c(5.1,5.1,4.1,2.1))
