...
bfile=/exports/igmm/eddie/wilson-lab/data/base_data/ukbb_19655/genotypes/10k_unrelated_white_british_reference/ukbb_chr${CHR}_10000_random_unrelated_white_british
awk -v OFS="\t" 'NR == 1 {print "SNP","P"; next} 
                 NR == FNR {var_id[$1] = $11; next}
                 var_id[$2] > 0 {print $2, var_id[$2]}' \
<(gzread $SUMSTATS) $bfile.bim > ${OUTFILE}.snps.${CHR}.tsv
...
chrs=`gzread $sumstats | awk 'NR > 1 && chrs[$3] == 0 {chrs[$3]++; next} END {for (i in chrs) {print i}} '`
...
awk 'ARGIND < ARGC-1 {hits[$2]++; next} FNR == 1 || hits[$2] > 0' ${outfile}.*.clumped <(gzread $sumstats) \
